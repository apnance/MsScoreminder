//
//  StatManagerPerformanceTests.swift
//  Ms Scoreminder
//
...
@available(iOS 16.0, *)
final class StatManagerPerformanceTests: XCTestCase {
    ...
    /// 10  = 1,024,000
    /// ```
    private static let doublings            = 7
    ...
    ///```
    /// Date        Scoreminder App Version     Test Total Runtime
    /// 10/23/24    1.6.2                       9m 47s (587.756s)
    /// ```
    func testDoublingRatios() {
        ...
        let totalRunTime = ContinuousClock().measure {
            ...
            let countToCSV: [Int:CSV] = Self.buildData()
            
            print("""
                    
                    ------------------------------------------------------------
                                        Doubling Ratios(x\(Self.doublings))
                    - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            ...
            var dataCount       = Self.startingScoreCount
            var data = ""
            var doublingCount = 0
            
            while dataCount <= Self.maxRandomScoreCount {
